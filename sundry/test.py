...
CTAS_QUERY_FMT = '''
CREATE TABLE {new_database}.tmp_{new_table_name}
WITH (
...
def drop_tmp_table(athena_client, now_timestamp, prev_timestamp):
    ...
    prev_table_name = '{table}_{year}{month:02}{day:02}{hour:02}{minute:02}'.format(table=NEW_TABLE_NAME,
...
def run_ctas(athena_client, now_timestamp, prev_timestamp):
    ...
    now_timestamp.year, now_timestamp.month, now_timestamp.day, now_timestamp.hour, now_timestamp.minute)

    new_table_name = '{table}_{year}{month:02}{day:02}{hour:02}{minute:02}'.format(table=NEW_TABLE_NAME, year=year,
    ...
    now = '{year}-{month:02}-{day:02} {hour:02}:{minute:02}'.format(year=year, month=month, day=day, hour=hour,
                                                                    minute=minute)
    prev = '{year}-{month:02}-{day:02} {hour:02}:{minute:02}'.format(year=prev_timestamp.year,
                                                                     month=prev_timestamp.month, day=prev_timestamp.day,
                                                                     hour=prev_timestamp.hour,
                                                                     minute=prev_timestamp.minute)
