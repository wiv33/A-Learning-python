...
class DriverDownloader:
    ...
    def _download(self):
        df = pd.read_csv(f'{self.driver}.csv', index_col='keys')
        os.system(f'rm -f {self.driver}*.zip')
        os.system(f"wget -O chromedriver.zip http://chromedriver.storage.googleapis.com/`curl -sS "
                  f"chromedriver.storage.googleapis.com/LATEST_RELEASE`/{df.loc[self._platform].post_fix}")
